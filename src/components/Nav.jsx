  ...
  return (
    ...
    >
      <motion.ul className="flex items-center justify-center space-x-[33px] px-[21px] w-fit lg:bg-[#FFFFFF] lg:bg-opacity-5 bg-[#F46C38] text-[#FFFFFF] lg:my-7 my-2 rounded-2xl py-3 relative ">
        {navItems.map(({ icon: Icon, name, path, newTab }, index) => (
          <Link
            to={path}
            key={index}
          ...
          >
            ...
            >
              ...
              <motion.span className="absolute bg-[#ffffff] bg-opacity-5 text-white rounded-lg px-2 py-1 opacity-0 group-hover:opacity-100 duration-700 top-14">
                {name}
              </motion.span>
            </motion.li>
          </Link>
